...
> [!info] 阅读导航
>
> - 所属主题：工程与落地 · Codex 专题（选读）
> - 前置阅读：[[AI 开发/基础概念/AI 开发核心概念|行动与编排的核心概念]]
> - 本文目标：理解 Codex 如何发现、合并和限制项目规则
> - 下一步：[[AI 开发/Agent 工程/Codex/多个项目如何复用 Codex 规则|多个项目如何复用 Codex 规则]]

## 1. AGENTS.md 解决什么问题

本文为产品选读，依据 2026 年 9 月 15 日核对的官方说明。第一遍建立全貌时，可以先读完知识地图的六个主题，再回来查具体加载规则。
...
它适合保存相对稳定、经常适用的约束。只在特定任务中使用的详细流程，更适合交给 [[AI 开发/Agent 工程/Codex/Codex Skill、Plugin、MCP 与 Superpowers|Codex Skill、Plugin、MCP 与 Superpowers]]。

## 2. 规则加载链

按官方描述，Codex 在运行开始时构建指令链；TUI 通常对应一次启动会话。它首先读取全局规则，然后从项目根目录沿当前工作目录逐层查找项目规则；找不到项目根目录时，只检查当前目录。不要假定编辑规则文件后，已运行会话必然立即重新加载。
...
### 6.2 override 可以压过所有子目录

不会。它只替换同一目录的普通文件，下层目录仍然可以添加更具体的规则。

### 6.3 任意子目录中的 AGENTS.md 都会加载
